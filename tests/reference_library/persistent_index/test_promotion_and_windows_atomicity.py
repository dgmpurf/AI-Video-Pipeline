...
def _two_generations(tmp_path: Path, ledger_harness):
    state = tmp_path / "state"
    first = build_generation(state, ledger_harness.mapped())
    ledger_harness.append("REVIEW_OBSERVATION_ADDED")
    second = build_generation(state, ledger_harness.mapped())
    return state, first, second


def test_initial_pointer_creation_and_immediate_reopen(tmp_path: Path, ledger_harness):
    state = tmp_path / "state"
    generation = build_generation(state, ledger_harness.mapped())
    pointer = promote_generation(generation.generation_path)
...
def test_reader_holding_old_generation_survives_pointer_replacement(
    ...
    state, first, second = _two_generations(tmp_path, ledger_harness)
    promote_generation(first.generation_path)
    old_reader = open_read_only(first.generation_path)
    try:
        promote_generation(second.generation_path)
...
def test_failed_pointer_replace_preserves_prior_pointer(
    ...
    with pytest.raises(PromotionError, match="atomic pointer promotion failed"):
        promote_generation(second.generation_path, replace_operation=fail_replace)
...
def test_pointer_hash_mismatch_fails_closed(tmp_path: Path, ledger_harness):
    state = tmp_path / "state"
    generation = build_generation(state, ledger_harness.mapped())
    pointer = dict(promote_generation(generation.generation_path))
