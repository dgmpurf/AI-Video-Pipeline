...
from app.ai_video_pipeline.execution.dreamina_f6_live import (
    CommandResult,
    ProductionOneShotAuthorization,
    build_production_submit_argv,
    execute_f6_live_run,
)
from app.ai_video_pipeline.execution.runner import PipelineRunner

ROOT = Path(__file__).resolve().parents[1]
REPORT = ROOT / "reports" / "PHASE_F6_PRODUCTION_TEXT2IMAGE_LIVE_RUN_REPORT.md"
...
def _workspace(tmp_path: Path) -> Path:
    ...
    fs_plan = json.loads((ROOT / "reports" / "A-SC-TEMPLE-COURTYARD-001_fs_write_plan.json").read_text(encoding="utf-8"))
    ...
    (workspace / "reports" / "A-SC-TEMPLE-COURTYARD-001_fs_write_plan.json").write_text(json.dumps(fs_plan, ensure_ascii=True, indent=2), encoding="utf-8")
    shutil.copy2(ROOT / "reports" / "A-SC-TEMPLE-COURTYARD-001_LIVE_APPROVAL_CHECKLIST.json", workspace / "reports" / "A-SC-TEMPLE-COURTYARD-001_LIVE_APPROVAL_CHECKLIST.json")
    return workspace
...
def test_01_one_shot_authorization_applies_only_to_exact_task_id() -> None:
    auth = ProductionOneShotAuthorization.required()
...
def test_03_production_text2image_live_argv_does_not_include_output_name_or_output_dir() -> None:
    argv = build_production_submit_argv(_task(), "dreamina.exe")
    assert "--output_name" not in argv
    assert "output_dir" not in " ".join(argv)


def test_04_query_result_is_not_in_submit_argv() -> None:
    assert "query_result" not in build_production_submit_argv(_task(), "dreamina.exe")
...
def test_05_download_dir_is_only_used_in_download_stage(tmp_path: Path) -> None:
    ...
    def runner(argv):
        ...
        if argv[1] == "text2image":
            return CommandResult(list(argv), 0, '{"submit_id":"PROD123"}', "")
        ...
        Image.new("RGB", (160, 160), color=(40, 60, 80)).save(download_dir / "raw.png")
        return CommandResult(list(argv), 0, '{"status":"downloaded"}', "")

    result = execute_f6_live_run(workspace, runner=runner)
    assert result.verdict == "PHASE_F6_SUCCESS_DOWNLOADED"
...
def test_06_submit_id_persistence_happens_before_query(tmp_path: Path) -> None:
    ...
    def runner(argv):
        ...
        current = run_dirs[-1]
        evidence["submitted_before_query"] = "PROD123" in (current / "job_store.json").read_text(encoding="utf-8")
        return CommandResult(list(argv), 0, '{"status":"querying"}', "")

    execute_f6_live_run(workspace, runner=runner)
...
def test_07_querying_state_resumes_query_only(tmp_path: Path) -> None:
    ...
    result = execute_f6_live_run(workspace, runner=runner)
    assert result.verdict == "PHASE_F6_SUBMITTED_QUERYING"
...
def test_08_success_state_downloads_and_renames_only_inside_workspace(tmp_path: Path) -> None:
    ...
    assert result.output_path.name == "A-SC-TEMPLE-COURTYARD-001_ancient_temple_rain_courtyard.png"
