...
from app.ai_video_pipeline.execution.dreamina_h4_live import (
    CommandResult,
...
REPORT = ROOT / "reports" / "PHASE_H4_SHOT01_KEYFRAME002_IMAGE2IMAGE_LIVE_RUN_REPORT.md"
MANIFEST = ROOT / "productions" / PRODUCTION_ID / "manifests" / "production_image2image_SHOT-01-KF-002.csv"
PROMPT = ROOT / "productions" / PRODUCTION_ID / "prompts" / "shot_01_keyframe_prompt_SHOT-01-KF-002.json"
FS_PLAN = ROOT / "reports" / "SHOT-01-KF-002_fs_write_plan.json"
CHECKLIST = ROOT / "reports" / "SHOT-01-KF-002_LIVE_APPROVAL_CHECKLIST.json"
REF_IDS = [
...
def _workspace(tmp_path: Path) -> Path:
    workspace = tmp_path.parent / f"h4ws_{uuid.uuid4().hex[:12]}"
    ...
    shutil.copy2(PROMPT, workspace / "productions" / PRODUCTION_ID / "prompts" / PROMPT.name)
    shutil.copy2(FS_PLAN, workspace / "reports" / FS_PLAN.name)
    shutil.copy2(CHECKLIST, workspace / "reports" / CHECKLIST.name)
    records = []
    for index, logical_id in enumerate(REF_IDS, start=1):
        ...
        _make_image(path, (240 + index * 10, 220 + index * 10), index * 13)
...
def test_03_live_argv_uses_image2image_repeated_images_and_no_forbidden_flags(tmp_path: Path) -> None:
    ...
    def runner(argv):
        ...
        return CommandResult(list(argv), 0, '{"submit_id":"SUB-H4"}', "") if argv[1] == "image2image" else CommandResult(list(argv), 0, '{"status":"querying"}', "")

    execute_h4_live_run(workspace, runner=runner)
    argv = calls[0]
    joined = " ".join(argv)
...
def test_04_download_dir_is_only_used_in_download_stage(tmp_path: Path) -> None:
    ...
    def runner(argv):
        ...
        if argv[1] == "image2image":
            return CommandResult(list(argv), 0, '{"submit_id":"SUB-H4"}', "")
        ...
        _make_image(download_dir / "raw.png", (260, 260), 23)
        return CommandResult(list(argv), 0, '{"status":"downloaded"}', "")

    result = execute_h4_live_run(workspace, runner=runner)
    assert result.verdict == "PHASE_H4_SUCCESS_DOWNLOADED"
...
def test_06_submit_id_persistence_happens_before_query(tmp_path: Path) -> None:
    ...
    def runner(argv):
        ...
        evidence["persisted"] = "SUB-H4" in (run_dir / "job_store.json").read_text(encoding="utf-8")
        return CommandResult(list(argv), 0, '{"status":"querying"}', "")
...
def test_07_querying_state_stops_after_single_query(tmp_path: Path) -> None:
    workspace = _workspace(tmp_path)
    calls: list[list[str]] = []
    ...
    result = execute_h4_live_run(workspace, runner=runner)
    assert result.verdict == "PHASE_H4_SUBMITTED_QUERYING"
    assert len(calls) == 2
...
def test_08_success_state_downloads_and_renames_inside_workspace(tmp_path: Path) -> None:
    ...
    def runner(argv):
        ...
        if "--download_dir" not in argv:
            return CommandResult(list(argv), 0, '{"status":"success"}', "")
        download_dir = Path(argv[argv.index("--download_dir") + 1])
        _make_image(download_dir / "raw.png", (260, 260), 31)
        return CommandResult(list(argv), 0, '{"status":"downloaded"}', "")

    result = execute_h4_live_run(workspace, runner=runner)
    assert result.output_path is not None
    assert result.output_path.name == "SHOT-01-KF-002_temple_gate_courtyard_standoff_keyframe.png"
    assert result.output_path.is_relative_to(workspace)


def test_09_providers_json_remains_allow_live_run_false_and_no_batch_modes() -> None:
...
def test_10_phase_h4_report_has_no_prohibited_strings_when_present() -> None:
    if REPORT.exists():
