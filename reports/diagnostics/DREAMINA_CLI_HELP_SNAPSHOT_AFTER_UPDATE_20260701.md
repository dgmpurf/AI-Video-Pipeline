# Dreamina CLI Help Snapshot After Update - 2026-07-01

Sanitized help-only snapshot. No generation, query, download, list_task execution, media creation, auth token, session file, signed URL, or user_id is included.

- Linux install path: `/home/ke/.local/bin/dreamina`
- Installer metadata path: `/home/ke/.dreamina_cli/version.json`
- Installed skill path: `/home/ke/.dreamina_cli/dreamina/SKILL.md`
...
- omit --generate_num to generate 1 image
...
Examples:
  dreamina text2image --prompt="a cat portrait" --ratio=1:1 --resolution_type=2k --generate_num=4
exit_code=0
```
...
Flags:
      ...
      --video_resolution string   supported values by model: seedance2.0_vip -> 720p, 1080p, or 4k; all other models -> 720p
      --model_version string      supported values: seedance2.0, seedance2.0fast, seedance2.0_vip, seedance2.0fast_vip, seedance2.0mini; default: seedance2.0fast
      --poll int                  submit then poll query_result for up to N seconds at 1s intervals (0 disables polling)
...
Flags:
      ...
      --video_resolution string   advanced controls only; supported values by model: seedance2.0_vip -> 720p, 1080p, or 4k; all other models -> 720p
      --model_version string      advanced controls only; supported values: seedance1.0fast, seedance1.0, seedance1.5pro, seedance2.0, seedance2.0fast, seedance2.0_vip, seedance2.0fast_vip, seedance2.0mini
      --session int               session id (default 0 "默认对话")
      --poll int                  submit then poll query_result for up to N seconds at 1s intervals (0 disables polling)
...
Flags:
      ...
      --prompt string             generation prompt
      --session int               session id (default 0 "默认对话")
      --duration int              video duration in seconds; supported ranges: seedance1.5pro -> 4-12, seedance2.0 family/seedance2.0mini -> 4-15 (default 5)
      --video_resolution string   supported values by model: seedance2.0_vip -> 720p, 1080p, or 4k; all other models -> 720p
      --model_version string      supported values: seedance1.5pro, seedance2.0, seedance2.0fast, seedance2.0_vip, seedance2.0fast_vip, seedance2.0mini; default: seedance2.0_vip
      --poll int                  submit then poll query_result for up to N seconds at 1s intervals (0 disables polling)
...
Upload local images, videos, and audio, then submit Dreamina's flagship multimodal video generation mode. This corresponds to the "全能参考" (All-around reference) feature on the web interface (formerly known as ref2video). This is the strongest video generation mode currently exposed in the CLI, supports all-around references, and supports the Seedance 2.0 family plus seedance2.0mini (flag values: seedance2.0, seedance2.0fast, seedance2.0_vip, seedance2.0fast_vip, seedance2.0mini). The task is asynchronous, but --poll can wait briefly before falling back to query_result.
...
Flags:
      ...
      --ratio string              supported values: 1:1, 3:4, 16:9, 4:3, 9:16, 21:9
      --video_resolution string   supported values by model: seedance2.0_vip -> 720p, 1080p, or 4k; all other models -> 720p
      --model_version string      supported values: seedance2.0, seedance2.0fast, seedance2.0_vip, seedance2.0fast_vip, seedance2.0mini
      --session int               session id (default 0 "默认对话")
      --poll int                  submit then poll query_result for up to N seconds at 1s intervals (0 disables polling)
