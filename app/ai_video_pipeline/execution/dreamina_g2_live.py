...
MANIFEST_NAME = "production_text2image_A-CH-A-SUBJECT-001.csv"
PROMPT_RECORD_NAME = "production_character_subject_prompt_A-CH-A-SUBJECT-001.json"
CHECKLIST_NAME = "A-CH-A-SUBJECT-001_LIVE_APPROVAL_CHECKLIST.json"
FS_PLAN_NAME = "A-CH-A-SUBJECT-001_fs_write_plan.json"
...
@dataclass
class CharacterSubjectOneShotAuthorization:
    task_id: str
    provider: str
    task_type: str
    user_confirmed: bool
    ...
    @classmethod
    def required(cls) -> "CharacterSubjectOneShotAuthorization":
        return cls(
            task_id=TASK_ID,
...
def load_g2_task(manifest_path: Path) -> TaskSpec:
    tasks = parse_manifest_csv(manifest_path)
    if len(tasks) != 1:
        raise ValueError("G2 manifest must contain exactly one task")
    return tasks[0]


def build_character_subject_submit_argv(task: TaskSpec, executable: str) -> list[str]:
...
def run_pre_submit_checks(
    *,
    ...
    authorization: CharacterSubjectOneShotAuthorization,
) -> list[str]:
    ...
    require(task.phase == "asset_character", "phase is asset_character")
    ...
    require(task.ratio == "9:16", "ratio is 9:16")
    ...
    require(prompt_record.get("asset_scope") == "production_character_subject", "prompt record is production character subject")
    require(prompt_record.get("prompt_text") == task.prompt, "manifest prompt matches prompt record")
    ...
    require(_flag_value(argv, "--ratio") == "9:16", "argv includes ratio 9:16")
    ...
    _validate_write_plan(fs_write_plan_path, root, policy, require)
    all_paths_text = " ".join([joined, str(run_context.run_root)])
...
def execute_g2_live_run(
    workspace_root: Path,
    *,
    runner: Runner = default_subprocess_runner,
    authorization: CharacterSubjectOneShotAuthorization | None = None,
) -> G2LiveResult:
    root = Path(workspace_root).resolve()
    policy = PathPolicy(root)
    ...
    task = load_g2_task(manifest_path)
    auth = authorization or CharacterSubjectOneShotAuthorization.required()
    ...
    argv = build_character_subject_submit_argv(task, executable)
    ...
    if submit_result.returncode != 0 or not submit_id:
        ...
        return G2LiveResult("PHASE_G2_FAILED", run_context, submit_id, submit_result, None, None, "submit_failed", None, None, {}, pre_checks)
    ...
    if query_status == "querying":
        ...
        return G2LiveResult("PHASE_G2_SUBMITTED_QUERYING", run_context, submit_id, submit_result, query_result, None, query_status, None, None, {}, pre_checks)
    if query_status == "success":
        ...
        if download_result.returncode != 0:
            ...
            return G2LiveResult("PHASE_G2_FAILED", run_context, submit_id, submit_result, query_result, download_result, "download_failed", None, None, {}, pre_checks)
        ...
        return G2LiveResult("PHASE_G2_SUCCESS_DOWNLOADED", run_context, submit_id, submit_result, query_result, download_result, query_status, output_path, raw_download, integrity, pre_checks)

    _persist_failed_query(run_context, task, submit_id, query_result, query_status, policy)
    return G2LiveResult("PHASE_G2_FAILED", run_context, submit_id, submit_result, query_result, None, query_status, None, None, {}, pre_checks)
...
def check_image_integrity(path: Path) -> dict[str, Any]:
    ...
    if not info["exists"] or info["file_size"] <= 10 * 1024:
        return info
    info["sha256"] = hashlib.sha256(target.read_bytes()).hexdigest()
    with Image.open(target) as image:
        image.verify()
    with Image.open(target) as image:
        info.update({"openable": True, "width": image.width, "height": image.height, "format": image.format})
    return info


def _validate_write_plan(fs_write_plan_path: Path, root: Path, policy: PathPolicy, require: Callable[[bool, str], None]) -> None:
    ...
    for planned in planned_values:
        ...
        if candidate.is_absolute():
            try:
                ...
                raise RuntimeError(f"pre-submit check failed: planned output path outside workspace: {planned}") from exc
        require(True, f"planned output path is workspace-scoped: {planned}")
    all_planned = " ".join(planned_values)
    for old_token in OLD_PATH_TOKENS:
        require(old_token not in all_planned, f"old path token absent from planned outputs: {old_token}")


def _write_initial_artifacts(run_context: RunContext, task: TaskSpec, argv: list[str], auth: CharacterSubjectOneShotAuthorization, policy: PathPolicy) -> None:
    ...
    add_or_update_job(store, task.task_id, status="planned", provider=task.provider.value, output_name=task.output_name, last_event="g2_live_plan_created")
    save_job_store(run_context.job_store_json, store, policy)
    _write_authorization(run_context, auth, policy)
    _write_log(run_context, ["phase=G2", "mode=live", "status=pre_submit_checks_passed", f"task_id={task.task_id}"], policy)
...
def _persist_submitted(run_context: RunContext, task: TaskSpec, result: CommandResult, submit_id: str, argv: list[str], policy: PathPolicy) -> None:
    ...
    store = create_job_store(run_context.run_dir_name, PRODUCTION_ID, RunMode.live.value)
    add_or_update_job(store, task.task_id, status="submitted", provider=task.provider.value, provider_task_id=submit_id, submit_id=submit_id, output_name=task.output_name, last_event="g2_submit_persisted_before_query")
    save_job_store(run_context.job_store_json, store, policy)
    _write_log(run_context, ["phase=G2", "mode=live", "status=submitted", f"submit_id={submit_id}", "submit_persisted_before_query=true", f"submit_returncode={result.returncode}", "batch=false", "parallel=false", "retry=false", "auto_continue=false"], policy)
...
def _persist_failed_submit(run_context: RunContext, task: TaskSpec, result: CommandResult, policy: PathPolicy) -> None:
    ...
    store = create_job_store(run_context.run_dir_name, PRODUCTION_ID, RunMode.live.value)
    add_or_update_job(store, task.task_id, status="failed", provider=task.provider.value, output_name=task.output_name, last_event="g2_submit_failed_no_retry")
    save_job_store(run_context.job_store_json, store, policy)
    _write_log(run_context, ["phase=G2", "mode=live", "status=submit_failed", f"submit_returncode={result.returncode}", "retry=false"], policy)
...
def _persist_querying(run_context: RunContext, task: TaskSpec, submit_id: str, result: CommandResult, policy: PathPolicy) -> None:
    ...
    store = create_job_store(run_context.run_dir_name, PRODUCTION_ID, RunMode.live.value)
    add_or_update_job(store, task.task_id, status="querying", provider=task.provider.value, provider_task_id=submit_id, submit_id=submit_id, output_name=task.output_name, last_event="g2_querying_stop_no_download")
    save_job_store(run_context.job_store_json, store, policy)
    _write_log(run_context, ["phase=G2", "mode=live", "status=querying", f"submit_id={submit_id}", "download=false", "retry=false"], policy)
...
def _persist_failed_query(run_context: RunContext, task: TaskSpec, submit_id: str, result: CommandResult, status: str, policy: PathPolicy) -> None:
    ...
    store = create_job_store(run_context.run_dir_name, PRODUCTION_ID, RunMode.live.value)
    add_or_update_job(store, task.task_id, status="failed", provider=task.provider.value, provider_task_id=submit_id, submit_id=submit_id, output_name=task.output_name, last_event=f"g2_{status}_no_retry")
    save_job_store(run_context.job_store_json, store, policy)
    _write_log(run_context, ["phase=G2", "mode=live", f"status={status}", f"submit_id={submit_id}", "retry=false"], policy)
...
def _persist_success(run_context: RunContext, task: TaskSpec, submit_id: str, query: CommandResult, download: CommandResult, raw_download: Path, output: Path, integrity: dict[str, Any], policy: PathPolicy) -> None:
    ...
    store = create_job_store(run_context.run_dir_name, PRODUCTION_ID, RunMode.live.value)
    add_or_update_job(store, task.task_id, status="completed", provider=task.provider.value, provider_task_id=submit_id, submit_id=submit_id, output_name=task.output_name, last_event="g2_download_renamed_integrity_checked")
    save_job_store(run_context.job_store_json, store, policy)
    _write_log(run_context, ["phase=G2", "mode=live", "status=completed", f"submit_id={submit_id}", f"output_path={output}", f"pillow_openable={integrity.get('openable')}", "retry=false"], policy)


def _write_authorization(run_context: RunContext, auth: CharacterSubjectOneShotAuthorization, policy: PathPolicy) -> None:
