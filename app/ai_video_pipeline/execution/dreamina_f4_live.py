...
TASK_ID = "DREAMINA-PREFLIGHT-TEXT2IMAGE-001"
PRODUCTION_ID = "chi_yan_tian_qiong"
OUTPUT_NAME = "DREAMINA_PREFLIGHT_TEXT2IMAGE_001"
...
def default_subprocess_runner(argv: Sequence[str]) -> CommandResult:
    try:
        ...
        return CommandResult(argv=list(argv), returncode=127, stdout="", stderr=str(exc))


def load_f4_task(manifest_path: Path) -> TaskSpec:
    tasks = parse_manifest_csv(manifest_path)
    if len(tasks) != 1:
        raise ValueError("F4 manifest must contain exactly one task")
    return tasks[0]


def build_text2image_submit_argv(task: TaskSpec, executable: str) -> list[str]:
...
def run_pre_submit_checks(
    *,
    ...
    require(task.phase == "preflight", "phase is preflight")
    ...
    require(task.prompt.startswith("A simple cinematic ancient temple courtyard"), "prompt matches F4 preflight task")
    ...
    require(policy.ensure(checklist_path).exists(), "manual approval checklist exists")
...
def execute_f4_live_run(
    ...
    *,
    ...
    root = Path(workspace_root).resolve()
    policy = PathPolicy(root)
    manifest_path = root / "productions" / PRODUCTION_ID / "manifests" / "dreamina_preflight_text2image_001.csv"
    fs_plan_path = root / "reports" / "dreamina_preflight_text2image_001_fs_write_plan.json"
    checklist_path = root / "reports" / "DREAMINA_PREFLIGHT_TEXT2IMAGE_001_LIVE_APPROVAL_CHECKLIST.json"
    providers_config_path = root / "configs" / "providers.json"
    providers_config = json.loads(policy.ensure(providers_config_path).read_text(encoding="utf-8"))
    executable = providers_config["providers"]["dreamina_cli"]["executable"]
    task = load_f4_task(manifest_path)
    ...
    artifacts = _artifact_paths(run_context)
    ...
    argv = build_text2image_submit_argv(task, executable)
    ...
    with download_execution.postprocessing_guard(stage="download_postprocessing"):
        ...
        if download_result.returncode != 0:
            ...
            return F4LiveResult("PHASE_F4_FAILED", run_context, submit_id, submit_result, query_result, download_result, "download_failed", None, {}, pre_checks)
        raw_download = find_downloaded_file(run_context.downloads_dir)
        output_path = rename_downloaded_image(raw_download, run_context.output_dir / f"{task.output_name}.png", policy)
...
def extract_submit_id(text: str) -> str:
    ...
    for payload in candidates:
        ...
        if found:
            return found
    patterns = [
        r"submit[_ -]?id[\"':= ]+([A-Za-z0-9._:-]+)",
        r"task[_ -]?id[\"':= ]+([A-Za-z0-9._:-]+)",
    ]
...
def find_downloaded_file(downloads_dir: Path) -> Path:
    files = sorted([path for path in downloads_dir.iterdir() if path.is_file()], key=lambda path: path.stat().st_mtime, reverse=True)
    if not files:
...
def _write_initial_artifacts(run_context: RunContext, task: TaskSpec, argv: list[str], auth: OneShotLiveAuthorization, policy: PathPolicy) -> None:
    ...
    add_or_update_job(store, task.task_id, status="planned", provider=task.provider.value, output_name=task.output_name, last_event="live_plan_created")
    save_job_store(run_context.job_store_json, store, policy)
    _write_authorization(run_context, auth, policy)
    _write_log(run_context, ["mode=live", "status=pre_submit_checks_passed", f"task_id={task.task_id}"], policy)
...
def _persist_submitted(run_context: RunContext, task: TaskSpec, result: CommandResult, submit_id: str, argv: list[str], policy: PathPolicy) -> None:
    ...
    store = create_job_store(run_context.run_dir_name, PRODUCTION_ID, RunMode.live.value)
    add_or_update_job(store, task.task_id, status="submitted", provider=task.provider.value, provider_task_id=submit_id, submit_id=submit_id, output_name=task.output_name, last_event="submit_persisted_before_query")
    save_job_store(run_context.job_store_json, store, policy)
    _write_log(run_context, ["mode=live", "status=submitted", f"submit_id={submit_id}", "submit_persisted_before_query=true", f"submit_returncode={result.returncode}", "batch=false", "parallel=false", "retry=false", "auto_continue=false"], policy)
...
def _artifact_paths(run_context: RunContext) -> dict[str, Path]:
    return {
        "run_manifest_snapshot": run_context.run_manifest_snapshot_csv,
        "run_plan": run_context.run_plan_json,
        "provider_requests": run_context.provider_requests_jsonl,
        "provider_responses": run_context.provider_responses_jsonl,
        "submitted_tasks": run_context.submitted_tasks_csv,
        "querying_tasks": run_context.querying_tasks_csv,
        "completed_tasks": run_context.completed_tasks_csv,
        "failed_tasks": run_context.failed_tasks_csv,
        "downloaded_files": run_context.downloaded_files_csv,
        "execution_log": run_context.execution_log_txt,
        "job_store": run_context.job_store_json,
        "command_preview": run_context.command_preview_csv,
    }
