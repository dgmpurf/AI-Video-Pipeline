...
MANIFEST_NAME = "production_image2image_SHOT-01-KF-002.csv"
PROMPT_NAME = "shot_01_keyframe_prompt_SHOT-01-KF-002.json"
REPORT_NAME = "PHASE_H4_SHOT01_KEYFRAME002_IMAGE2IMAGE_LIVE_RUN_REPORT.md"
REFERENCE_IDS = [
...
def default_subprocess_runner(argv: Sequence[str]) -> CommandResult:
    ...
    return CommandResult(list(argv), int(completed.returncode), completed.stdout or "", completed.stderr or "")


def execute_h4_live_run(
    workspace_root: Path,
    *,
    ...
    task = load_h4_task(manifest_path, prompt_path)
    ...
    write_json(run_context.input_media_dir / "input_media_manifest.json", {"task_id": TASK_ID, "references": staged_refs}, policy)
    ...
    try:
        ...
        result = H4LiveResult("PHASE_H4_REJECTED_BEFORE_SUBMIT", run_context, "", None, None, None, "pre_submit_rejected", None, None, {}, [], staged_refs, False, root / "reports" / REPORT_NAME)
    ...
    if submit_result.returncode != 0 or not submit_id:
        ...
        result = H4LiveResult("PHASE_H4_FAILED", run_context, submit_id, submit_result, None, None, "submit_failed", None, None, {}, pre_checks, staged_refs, upload_warning, root / "reports" / REPORT_NAME)
        _write_report(root, result, "pending", policy)
        return result
    ...
    if query_status == "querying":
        ...
        verdict = "PHASE_H4_SUBMITTED_WITH_UPLOAD_WARNING" if upload_warning else "PHASE_H4_SUBMITTED_QUERYING"
        result = H4LiveResult(verdict, run_context, submit_id, submit_result, query_result, None, query_status, None, None, {}, pre_checks, staged_refs, upload_warning, root / "reports" / REPORT_NAME)
        _write_report(root, result, "pending", policy)
        return result
    ...
    if query_status == "success":
        ...
        if download_result.returncode != 0:
            ...
            result = H4LiveResult("PHASE_H4_FAILED", run_context, submit_id, submit_result, query_result, download_result, "download_failed", None, None, {}, pre_checks, staged_refs, upload_warning, root / "reports" / REPORT_NAME)
            _write_report(root, result, "pending", policy)
            return result
        ...
        result = H4LiveResult("PHASE_H4_SUCCESS_DOWNLOADED", run_context, submit_id, submit_result, query_result, download_result, query_status, output_path, raw_download, integrity, pre_checks, staged_refs, upload_warning, root / "reports" / REPORT_NAME)
        _write_report(root, result, "pending", policy)
        return result

    _persist_failed_query(run_context, task, submit_id, query_result, query_status, responses, policy)
    result = H4LiveResult("PHASE_H4_FAILED", run_context, submit_id, submit_result, query_result, None, query_status, None, None, {}, pre_checks, staged_refs, upload_warning, root / "reports" / REPORT_NAME)
    _write_report(root, result, "pending", policy)
    return result


def load_h4_task(manifest_path: Path, prompt_path: Path) -> TaskSpec:
    tasks = parse_manifest_csv(manifest_path)
    if len(tasks) != 1:
        raise ValueError("H4 manifest must contain exactly one task")
    task = tasks[0]
...
def stage_ordered_references(root: Path, run_context: RunContext, reference_ids: list[str], policy: PathPolicy) -> list[dict[str, Any]]:
    ...
    for index, logical_id in enumerate(REFERENCE_IDS, start=1):
        record = _registry_record(registry.get("assets", []), logical_id)
...
def run_pre_submit_checks(
    *,
    ...
    require(task.task_id == TASK_ID, "task_id is SHOT-01-KF-002")
    ...
    require("mid-ground action zone" in task.prompt and "three-quarter front-side" in task.prompt, "prompt includes three-layer positioning correction")
    ...
    for item in staged_refs:
        ...
        require(Path(item["staged_path"]).name.isascii(), f"staged filename is ASCII: {item['logical_id']}")
    ...
    require(fs_plan.get("task_id") == TASK_ID, "fs_write_plan is for SHOT-01-KF-002")
    for operation in fs_plan.get("planned_operations", []):
        if isinstance(operation, dict):
            require(operation.get("executed") is False, f"fs_write_plan operation remains unexecuted: {operation.get('target', '')}")
    ...
    require(providers_config["providers"]["dreamina_cli"].get("allow_live_run") is False, "providers.json remains allow_live_run false")
    return checks
...
def _registry_record(records: list[dict[str, Any]], logical_id: str) -> dict[str, Any]:
    matches = [item for item in records if item.get("logical_id") == logical_id and item.get("status") == "locked_after_human_review"]
    if len(matches) != 1:
        raise RuntimeError(f"pre-submit check failed: expected one locked registry record for {logical_id}")
    return matches[0]
...
def _write_initial_artifacts(
    ...
    add_or_update_job(store, task.task_id, status="planned", provider=task.provider.value, output_name=task.output_name, last_event="h4_pre_submit_checks_passed")
    ...
    _write_authorization(run_context, auth, policy)
    _write_log(run_context, ["mode=live", "phase=H4", "status=pre_submit_checks_passed", f"task_id={task.task_id}"], policy)
...
def _write_report(root: Path, result: H4LiveResult, pytest_result: str, policy: PathPolicy) -> Path:
    ...
    lines = [
        ...
        "- result: `PASS`" if result.pre_submit_checks else "- result: `REJECTED_BEFORE_SUBMIT`",
        "",
        "## 2) Live authorization result",
        "- scoped authorization applied to `SHOT-01-KF-002` only.",
        "- authorization consumed after submit: `true`" if result.submit_result else "- authorization consumed after submit: `false`",
        "- batch: `false`",
        "- parallel: `false`",
        "- retry: `false`",
        "- auto_continue: `false`",
    ...
    if result.query_status == "success":
        lines.extend(
            [
                "",
                "## 8) Download result",
                f"- raw_download_path: `{result.raw_download_path or ''}`",
                f"- renamed output path: `{result.output_path or ''}`",
...
def main() -> None:
    result = execute_h4_live_run(Path.cwd())
